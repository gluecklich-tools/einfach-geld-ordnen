...
## RESUME - 20260426 START/BEDIENUNG USED RANGE CLEANUP

### Current active task
START_BEDIENUNG_USED_RANGE_CLEANUP_SCAN_ONLY on:
C:\Users\carst\Projekte\Claude\Excel\Kopie von EGO_VOLLVERSION_PREMIUM_DESIGN_20260425.xlsx

### Active sheets
- START
- BEDIENUNG

### Explicitly forbidden sheet
- HAUSHALTSBUCH: do not touch, do not normalize, do not clean, do not reformat.

### Last verified problem state
- START UsedRange: A:AU / LastRow 1048576 / prior COM RealLast A3.
- BEDIENUNG UsedRange: A1:X200 / LastRow 200 / prior COM RealLast D1.
- HAUSHALTSBUCH UsedRange: A1:AB20003 / LastRow 20003 / prior COM RealLast R1 / no-touch.

### Drift correction
The old HAUSHALTSBUCH and PRO-Recovery scope in older control files is superseded for this local 20260425 workbook cleanup. The only current task is merge-safe UsedRange cleanup preparation for START and BEDIENUNG.

### Hard learning before continuing
The failed cleanup used ClearFormats/ClearContents on ranges that intersected merged cells. For this task, COM-clear operations are blocked. The next apply must use exact OpenXML bounds cleanup after backup and merge map verification.

### Next step
1. Confirm governance/tooling sync report.
2. Create exact workbook backup.
3. Create one OpenXML cleanup step for START and BEDIENUNG only.
4. Verify XML validity, workbook openability, UsedRange/bounds, merge preservation, HAUSHALTSBUCH hash/no-touch.
5. Report and sync.
