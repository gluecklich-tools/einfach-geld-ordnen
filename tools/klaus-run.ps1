...
# EGO_AUTOSITEMAP_PRETTY_V2
try{
  $p = Join-Path $repo 'sitemap.xml'
  if(Test-Path -LiteralPath $p){
    $raw = [IO.File]::ReadAllText($p,[Text.UTF8Encoding]::new($false)).TrimStart([char]0xFEFF).Trim()
    [xml]$xd = $raw
    ...
    $s.Encoding = [Text.UTF8Encoding]::new($false)
    $s.OmitXmlDeclaration = $false
    $xw = [System.Xml.XmlWriter]::Create($p,$s)
    $xd.Save($xw)
    $xw.Close()
  }
}catch{
  Write-Warning ("Auto-Sitemap pretty failed: {0}" -f ($_.Exception.Message))
}
# EGO_AUTOSITEMAP_PRETTY_V1
try{
  $p=Join-Path $repo 'sitemap.xml'
  if(Test-Path -LiteralPath $p){
    $raw=[IO.File]::ReadAllText($p,[Text.UTF8Encoding]::new($false)).TrimStart([char]0xFEFF).Trim()
  ...
  Write-Warning ("Auto-Sitemap pretty failed: {0}" -f (#requires -Version 7.0
param(
  [Parameter(Mandatory=$false)]
  [ValidateSet("default","live-check")]
  [string]$Mode = "default",
...
try{
  ...
  if(Test-Path -LiteralPath $p){
    ...
    $xw=[System.Xml.XmlWriter]::Create($sw,$s)
    $xd.Save($xw); $xw.Close()
    [IO.File]::WriteAllText($p,$sw.ToString(),[Text.UTF8Encoding]::new($false))
  }
}catch{ Write-Warning ("Auto-Sitemap pretty failed: {0}" -f #requires -Version 7.0
param(
  [Parameter(Mandatory=$false)]
  [ValidateSet("default","live-check")]
  [string]$Mode = "default",
...
Say "DONE."
git status --porcelain.Exception.Message) }
}
